...
std::shared_ptr<std::vector<byte>> Hash(std::vector<byte> const &message)
{
	...
	try
	{
		...
		// Calculate the sizes
		status = BCryptGetProperty(hAlgorithm, BCRYPT_OBJECT_LENGTH, reinterpret_cast<PBYTE>(&sizeHashObject), sizeof(DWORD), &sizeData, 0);
		if (status != STATUS_SUCCESS)
		{
			throw;
		}

		status = BCryptGetProperty(hAlgorithm, BCRYPT_HASH_LENGTH, reinterpret_cast<PUCHAR>(&sizeHash), sizeof(DWORD), &sizeData, 0);
		if (status != STATUS_SUCCESS)
		{
			throw;
		}
		...
		// Create the hash
		status = BCryptCreateHash(hAlgorithm, &hHash, hashObject.get(), sizeHashObject, nullptr, 0, 0);
		if (status != STATUS_SUCCESS)
		{
			throw;
		}
		...
		// Hash the data
		status = BCryptHashData(hHash, (PUCHAR)message.data(), static_cast<ULONG>(message.size()), 0);
		if (status != STATUS_SUCCESS)
		{
			throw;
		}
